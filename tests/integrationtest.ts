...
describe("Health Records Complete Flow Test", () => {
  ...
  it("Should complete the entire flow: init counter, register user, register org, upload 4 records, grant access to first 3", async () => {
    const userKeypair = Keypair.generate();
    const organizationOwnerKeypair = Keypair.generate();
    const organization2OwnerKeypair = Keypair.generate();
    const teeNodeKeypair = Keypair.generate();
    ...
    // Health record PDAs (for record IDs 0-3, since counter starts at 1 but records use 0-based indexing)
    const [healthRecord1Pda, healthRecord1Bump] = PublicKey.findProgramAddressSync(
      [Buffer.from("health_record"), userKeypair.publicKey.toBuffer(), Buffer.from([1, 0, 0, 0, 0, 0, 0, 0])],
      program.programId
    );
    ...
    const registerUserTx = await program.methods
      .registerUser(userName, new anchor.BN(userAge))
      .accountsStrict({
        userVault: userVaultPda,
        owner: userKeypair.publicKey,
        systemProgram: SystemProgram.programId,
    ...
    console.log("Register user transaction signature:", registerUserTx);

    // Verify the user was registered
    const userVaultAccount = await program.account.userVault.fetch(userVaultPda);
    assert.equal(userVaultAccount.name, userName);
    assert.equal(userVaultAccount.age.toNumber(), userAge);
    assert.equal(userVaultAccount.owner.toString(), userKeypair.publicKey.toString());
    assert.equal(userVaultAccount.isActive, true);
    assert.equal(userVaultAccount.recordIds.length, 0);
    console.log("✓ User registered:", userVaultAccount.name, "Age:", userVaultAccount.age.toNumber());

    console.log("\nStep 3: Registering organization...");
    const organizationName = "Test Healthcare Organization";
    const contactInfo = "contact@testhealthcare.com";
    const description = "this is the organization description";
    ...
    console.log("\nStep 5: Uploading 4 health records...");
    const healthRecords = [
      {
        title: "Blood Test Results",
        description: "Complete blood count and lipid panel",
    ...
    // Verify user vault was updated
    const updatedUserVaultAccount = await program.account.userVault.fetch(userVaultPda);
    assert.equal(updatedUserVaultAccount.recordIds.length, 4);
    assert.equal(updatedUserVaultAccount.owner.toString(), userKeypair.publicKey.toString());
    assert.equal(updatedUserVaultAccount.isActive, true);
    console.log("✓ User vault contains all 4 record IDs:", updatedUserVaultAccount.recordIds.map(id => id.toNumber()));

    const updatedRecordCounterAccount = await program.account.recordCounter.fetch(recordCounterPda);
    assert.equal(updatedRecordCounterAccount.recordId.toNumber(), 5);
    console.log("✓ Record counter incremented to:", updatedRecordCounterAccount.recordId.toNumber());

    console.log("\nStep 6: Granting organization access to first 3 records...");
    const recordsToGrantAccess = [
      { recordId: 1, pda: healthRecord1Pda },
      { recordId: 2, pda: healthRecord2Pda },
      { recordId: 3, pda: healthRecord3Pda },
    ...
    for (const record of recordsToGrantAccess) {
      const grantAccessTx = await program.methods
        .grantAccess(new anchor.BN(record.recordId), organizationOwnerKeypair.publicKey)
        .accountsStrict({
          healthRecord: record.pda,
          organization: organizationPda,
          owner: userKeypair.publicKey,
    ...
    console.log("Organization Name:", finalOrganizationAccount.name);
    console.log("User Vault Record IDs:", finalUserVaultAccount.recordIds.map(id => id.toNumber()));
    console.log("User Name:", finalUserVaultAccount.name);
    console.log("User Age:", finalUserVaultAccount.age.toNumber());
    ...
    allHealthRecords.forEach((record, index) => {
      console.log(`\nHealth Record ${index + 1}:`, {
        ...
        fileSize: record.data.fileSize.toNumber(),
        accessCount: record.data.accessList.length
      });
    });
    ...
    allOrganizations.forEach((org, index) => {
      console.log(`\nOrganization ${index + 1}:`, {
        ...
        description: org.account.description,
        contactInfo: org.account.contactInfo,
        owner: org.account.owner.toString(),
        createdAt: new Date(org.account.createdAt.toNumber() * 1000).toISOString()
      });
    });
    
    console.log("\n🎉 All tests passed! Complete flow executed successfully:");
    console.log("- ✓ Record counter initialized");
    console.log("- ✓ User registered");
    console.log("- ✓ Organizations registered");
    console.log("- ✓ TEE node registered");
    console.log("- ✓ 4 health records uploaded");
    console.log("- ✓ Access granted to first 3 records");
