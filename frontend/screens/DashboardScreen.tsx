...
const DashboardScreen = () => {
    ...
    const [registeredOrganization, setRegisteredOrganization] = useState<boolean>(false);
                ...
                for (const acc of accounts) {
                    ...
                    try {
                        ...
                        if (offset + titleLen > totalLength) {
                            console.log("📏 titleLen =", titleLen, "| remaining =", totalLength - offset);
                        ...
                        if (offset + descLen > totalLength) {
                            console.log("📏 descLen =", descLen, "| remaining =", totalLength - offset);
        ...
        async (name: string, contactInfo: string) => {
            return await transact(async (wallet: Web3MobileWallet) => {
                try {
                    ...
                    const discriminator = Buffer.from([183, 29, 228, 76, 94, 9, 196, 137]);
    ...
    const [registerOrganizationLoading, setRegisterOrganizationLoading] = useState<boolean>(false);
    ...
    return (
        <LinearGradient colors={['#001F3F', '#003366', '#001F3F']} style={styles.container}>
            <View style={styles.wrapper}>
                    ...
                    {isUser ? (
                        ...
                        <>
                            {registeredOrganization ? (
                                <>
                                    <StatCard title="Records Accessed" value={5} icon="visibility" />
                                </>
                            ) : (
                                <View style={styles.registrationForm}>
                                    <Text style={styles.formHeading}>
                                        Your organization is not registered. Please create an account to continue using the application.
                                    </Text>
                ...
                {/* Action Button */}
                {isUser && (
                    <TouchableOpacity style={styles.uploadButton} onPress={() => { navigate('Upload') }}>
